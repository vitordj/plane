...
| `TRUSTED_PROXIES`                             | **Yes**  | CIDR of the proxy network in front of the stack (Coolify). Caddy only believes `X-Forwarded-For` from these ranges; the Compose file refuses to start without it | _User-provided (e.g. `10.0.0.0/8`)_        |
