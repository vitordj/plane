...
| `TRUSTED_PROXIES`                             | **Yes**  | CIDR range(s) of the proxy in front of Caddy (the Coolify/ingress network), comma-separated. Only these sources may set the client IP via `X-Forwarded-For`; the stack refuses to start without it | _User-provided (e.g. `10.0.0.0/8`)_        |
