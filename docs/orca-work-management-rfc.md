...
**Status:** especificação em implementação. Revisão 13 (10/09/2026).
...
| Alocar ao integrante menos carregado                       | Sim                   | `apps/api/plane/app/services/orca/assignment_service.py` (`lb-2`, lock por área, decisão registrada); disparo manual; `fill_empty`/`append` deprecados em favor de `assignment_mode`                                                                                                                                                                                                       |
...
| Processos recorrentes / SLA lateral                        | Parcial               | Modelos `IssueServiceLevel`, `ProcessInstanceReference`, `ProcessInstanceItem`, `ProcessCompletionEvent` (migração `0142`, itens 4.2/4.3), flag `ORCA_PROCESS_PROJECTION_ENABLED` (default off). Orquestrador sidecar e agrupamento visual ainda são 4.4/4.6.                                                                                                                              |
| Disponibilidade, férias, capacidade                        | Sim                   | Modelos `WorkspaceMemberAvailability` e `MembershipAllocationSettings` (migração `0141`); `rank_candidates` (`lb-2`); endpoints `availability/me/`, `members/{id}/availability/`, `members/{pk}/allocation/`; UI de férias e teto; sweep horário `sweep_unavailable_executors` e signal em `IssueAssignee`. Flag `ORCA_AVAILABILITY_ENABLED` (default off). Gate 3 (staging) ainda aberto. |
...
| Reatribuição quando alguém sai                             | Parcial               | Sweep e signal devolvem à fila (`queue_reason=executor_unavailable`, `trigger=availability`); não escolhem outra pessoa. `QuerySet.delete()` nativo de assignees não dispara signal — o buraco continua sendo `audit_organizational_routing`.                                                                                                                                              |
...
| A5  | Verificar comportamento real do Plane Compose em re-push   | **Fechada 2026-09-10.** F12 confirmada: Compose versiona schema; instâncias pela API pública. Ver `docs/orca-compose-notes.md`.                                                                                              |
| A6  | Módulo por instância quando a instância atravessa projetos | **Fechada 2026-09-10 como "não na v1".** A identidade canônica é `ProcessInstanceReference` (F20); a fila agrupa por ela. Um `Module` nativo por instância fica para se um piloto de um só projeto precisar da vista nativa. |
...
| Data       | Mudança                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| ---------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 2026-09-10 | Rev. 15: **Fase 5 itens 5.1, 5.3 e 5.4** — `GET /api/orca/workspaces/{slug}/executive/` (Workspace Admin, cache 5 min) agrega por área e por processo; o drill-down omite itens de projeto sem `ProjectMember` e devolve `hidden_count`; página em settings; testes com dataset de 40 itens; SQL em `docs/orca-executive-metrics.md`. **5.2 fica aberto**: sem medição em staging (>2 s) não se cria `OrcaExecutiveSnapshot`. Nenhuma decisão F1–F24 tocada.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| 2026-09-10 | Rev. 14: **Fase 4 itens 4.1, 4.5, 4.6, 4.7.** A5 fechada: Compose (docs oficiais, `plane-compose` 0.5.2) autentica por connection/PAT, re-push com o mesmo `id` local atualiza o mesmo item remoto, `.plane/state.json` é só o mapa de sync, e não há campo de área nem custom property na CE 1.4.x — F12 confirmada, instâncias continuam na API pública (`docs/orca-compose-notes.md`). O webhook nativo de `issue` ganha sidecar `orca: {unit_slug, routing_state, primary_executor}` sem alterar `IssueExpandSerializer`. A fila e Minha Área agrupam por `ProcessInstanceReference` com progresso `n/m`. A6 (módulo nativo por instância) fica fora da v1. Runbook em `docs/orca-processes-runbook.md`. 4.4 continua outro repositório. Nenhuma decisão F1–F24 reaberta.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
| 2026-09-10 | Rev. 13: **Fase 4 itens 4.2 e 4.3** — migração `0142_orca_process_projection` cria `IssueServiceLevel`, `ProcessInstanceReference`, `ProcessInstanceItem` e `ProcessCompletionEvent` (a RFC falava 0142/0143; uma só 0142 com os três modelos, como a árvore original). Flag `ORCA_PROCESS_PROJECTION_ENABLED` (default off, parser estrito do P0.14). O serviço D0.5 preenche `IssueServiceLevel` quando `assignment_due_at`/`completion_due_at` chegam; `original_*` são imutáveis. `POST work-items/` aceita o bloco `process`; `POST .../complete/` fecha por `completion_mode` (`automatic` / `automatic_with_review` / `manual` → 409 `ORG_COMPLETION_MANUAL_ONLY`); `GET .../process-instances/{source}/{id}/` lê o progresso. Campos opcionais `completed_state` e `review_state` na política de alocação. Nenhuma decisão F1–F24 tocada. O esclarecimento da rev. 5 de que `completion_due_at` era recusado fecha: a `IssueServiceLevel` agora existe.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
| 2026-09-10 | Rev. 12: **Fase 3 itens 3.3–3.6** — endpoints de disponibilidade e alocação (códigos 4938–4943), UI de férias/teto, sweep horário que devolve à fila sem reatribuir, sugestão `accepted_suggestion`, testes de fechamento. Sem migração nova (`0141` já existia). Nenhuma decisão F1–F24 tocada.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| 2026-09-09 | Rev. 11: **Fase 3 item 3.2** — `rank_candidates` passa a `lb-2`: com `ORCA_AVAILABILITY_ENABLED` ligada, exclui indisponíveis (`unavailable`), opt-out por área (`opted_out`), teto pessoal (`member_limit`) e teto da política (`policy_limit`, o antigo `at_max_open_items`). Com a flag desligada os filtros novos são no-ops e quem é escolhido não muda; a decisão grava `algorithm_version=lb-2` mesmo assim, para o snapshot dizer qual código decidiu. Carga continua por executor: a pessoa indisponível sai do ranking, os itens dela não migram para a carga de outra. Nenhuma decisão F1–F24 tocada.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| 2026-09-09 | Rev. 10: **Fase 3 item 3.1** — tabelas `WorkspaceMemberAvailability` e `MembershipAllocationSettings` (migração `0141`), flag `ORCA_AVAILABILITY_ENABLED` (default off, parser estrito do P0.14), helpers `is_available` / `accepts_new_work` que respondem sempre permissivos com a flag desligada. Janela half-open `[from, until)`; `until` null é aberto; sobreposição permitida. A Fase 4 passa a `0142`/`0143`. Nenhuma decisão F1–F24 tocada.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
...
| `algorithm_version`                                    | `CharField(16)`; v1 = `"lb-1"` (D0); v2 = `"lb-2"` (Fase 3: disponibilidade, opt-out, limites)                               |
...
| `workspace`                             | FK                                                              |
| `unavailable_from`, `unavailable_until` | datetimes; `until` null = indefinido; half-open `[from, until)` |
...
**`IssueServiceLevel`** (migração `0142`, Fase 4)
...
**`ProcessInstanceReference`** e **`ProcessInstanceItem`** (migração `0143`, Fase 4)
...
### 6.4 Ranking `least_loaded` (algoritmo `lb-2`)
...
Elegíveis(unit, project):
    ...
    ∧ user.is_bot = false
    ∧ (Fase 3, se ORCA_AVAILABILITY_ENABLED) sem WorkspaceMemberAvailability cobrindo now()
    ∧ (Fase 3, se flag) MembershipAllocationSettings.accepts_new_work ≠ false
    ∧ (Fase 3, se flag e MembershipAllocationSettings.max_open_items) total_open < limite pessoal
    ∧ (se policy.max_open_items_per_member) total_open < limite da política
Carga por pessoa que passou a membership/projeto (não só pelos elegíveis finais):
...
Ordenar os elegíveis por (total_open ASC, unit_open ASC, last_auto ASC NULLS FIRST, user_id ASC)
Escolher o primeiro; se vazio → allocation_failed / no_eligible_member
```

`lb-2` é `lb-1` mais os filtros da Fase 3. A ordenação e a carga (só
executor principal, workspace antes da área, última automática, desempate
por id) não mudam. Com a flag desligada os filtros novos são no-ops, então
quem é escolhido é o mesmo de `lb-1`; a decisão mesmo assim grava
`algorithm_version = "lb-2"`.

Excluídos entram no `candidates_snapshot` com `excluded_reason`:
`already_assigned`, `not_a_project_member`, `project_role_too_low`, `bot`,
`unavailable`, `opted_out`, `member_limit`, `policy_limit`. Um item de
pessoa indisponível continua na carga **dela**; não é redistribuído para
os outros.
...
| 4.2  | Migrações `0142`/`0143`: `IssueServiceLevel`, `ProcessInstanceReference`, `ProcessInstanceItem`; flag `ORCA_PROCESS_PROJECTION_ENABLED`                                                                                                                  |
...
| Ranking `lb-2`        | menos carregado vence; empate total → menos na área; empate → nunca alocado antes de já alocado; empate → menor id; colaborador não conta; item concluído não conta; bot excluído; Guest excluído; teto da política (`policy_limit`); com a flag, férias (`unavailable`), opt-out (`opted_out`), teto pessoal (`member_limit`); flag off não muda quem é escolhido |
