...
| Disponibilidade, férias, capacidade                        | Parcial               | Modelos `WorkspaceMemberAvailability` e `MembershipAllocationSettings` (migração `0141`, item 3.1); `rank_candidates` (`lb-2`, item 3.2) exclui indisponíveis, opt-out e limites quando `ORCA_AVAILABILITY_ENABLED` está ligada. Endpoints/UI e sweep ainda são 3.3–3.4.                                                              |
