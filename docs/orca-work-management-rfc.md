...
| Papel `lead` na área                                       | Só rótulo          | `OrganizationalUnitMemberRole`; nenhuma permissão decorre disso. Quem tem permissões próprias é o **coordenador** (`OrganizationalUnitCoordinator`, Fase 2), que é tabela à parte                                                                                                                                                                                                                                                             |
...
| Tela da área                                               | Sim                | `unit-detail.tsx` com abas trabalho, pessoas, projetos, coordenação e política; fila em `unit-work-tab.tsx`; página `:workspaceSlug/my-areas` para quem não é admin (Fase 2)                                                                                                                                                                                                                                                                  |
...
| Disponibilidade, férias, capacidade                        | Sim, atrás de flag | `WorkspaceMemberAvailability` e `MembershipAllocationSettings`; helpers em `services/orca/availability.py`; varredura horária em `bgtasks/organizational_availability_task.py`. Tudo atrás de `ORCA_AVAILABILITY_ENABLED`, que nasce desligada (Fase 3)                                                                                                                                                                                       |
...
| Reatribuição quando alguém sai                             | Sim                | varredura horária devolve à fila o item de quem ficou indisponível, saiu da área, do workspace ou do projeto, ou foi retirado do item pelo campo nativo de assignees; um `AssignmentDecision(trigger=availability)` por item. Nunca escolhe substituto — a fila mostra sugestão, quem decide é pessoa                                                                                                                                         |
...
| Processos externos projetados no Plane                     | Sim, atrás de flag | `ProcessInstanceReference`, `ProcessInstanceItem`, `ProcessCompletionEvent` e `IssueServiceLevel` em `organizational_process.py`; bloco `process` na criação, `POST .../complete/` e leitura da instância em `api/views/orca/work_items.py`; agrupamento por instância na fila. Atrás de `ORCA_PROCESS_PROJECTION_ENABLED`, que nasce desligada (Fase 4). O orquestrador é serviço externo — contrato em `docs/orca-orchestrator-contract.md` |
| Webhook nativo carrega a área                              | Sim                | chave `orca` no payload de `issue` (`services/orca/webhook_payload.py`): área, `routing_state`, executor por id, prazo e passo do processo. Item sem área não ganha a chave                                                                                                                                                                                                                                                                   |
| Dashboard da área / executivo                              | Sim                | `services/orca/executive_metrics.py` (dez indicadores por área, percentis por `percentile_cont`, cache de 5 min), rotas `GET /api/orca/workspaces/{slug}/executive/` e `.../executive/drilldown/` (Workspace Admin), página em **Configurações do workspace → Visão executiva**. Definições e SQL reproduzível em `docs/orca-executive-metrics.md` (Fase 5)                                                                                   |
...
**A5 e A6 continuam abertas com a Fase 4 entregue.** A5 exige ler a
documentação oficial do Plane Compose, que o ambiente desta implementação não
alcança (a política de rede bloqueia a saída para `developers.plane.so`) —
precisa de alguém com acesso a ela, e a Fase 4 foi construída sob F12 sem
depender do resultado: schema por Compose, instâncias sempre pela API. A6
(módulo nativo por instância) ficou de fora porque é uma decisão de produto
sobre o que aparece no projeto de quem não trabalha por área, não uma lacuna de
implementação: o agrupamento por instância já existe na fila, sem criar
`Module` nenhum.
...
| Data       | Mudança                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
| ---------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| 2026-09-06 | Rev. 8: três esclarecimentos de mecanismo abertos pela implementação da Fase 5 (nenhuma decisão F1–F24 tocada). (1) **População vazia responde `null`, não `0`**: uma área que não segura nada não tem mediana de espera nem concentração, e imprimir zero leria como atendimento instantâneo e distribuição perfeita. (2) **O `auto_assign_kept_ratio` só conta reversão feita por pessoa** (`decided_by` preenchido): trabalho devolvido pela varredura de disponibilidade não é o ranking errando, e contá-lo pioraria o número toda vez que a Fase 3 fizesse seu trabalho. (3) **A contagem é do workspace, as linhas são de quem lê**: o agregado ignora acesso a projeto (um número que omitisse três projetos em silêncio seria pior que nenhum número), o detalhamento respeita `ProjectMember` (F18) e a diferença aparece na tela em vez de sumir.                                                                                                                                                                                                                                                                                                                                                             |
| 2026-09-06 | Rev. 7: quatro esclarecimentos de mecanismo abertos pela implementação da Fase 4 (nenhuma decisão F1–F24 tocada). (1) **Uma migração, não duas**: o plano previa `0141`/`0142`, mas as quatro tabelas de processo e os dois campos de estado na política entram juntos ou não entram — uma metade aplicada é um estado que ninguém consegue descrever. (2) **A reivindicação recusada também é gravada**: `complete/` num passo `manual` responde 409 e ainda assim escreve `ProcessCompletionEvent(applied=false)`, porque um robô que insiste em declarar concluído um passo que só uma pessoa fecha é um fato que alguém precisa conseguir ver. (3) **O `status` da instância é derivado dos itens, não lido da coluna**: a coluna é cache que o caminho de conclusão mantém, e quem fecha o último passo pela interface encerra a execução tão de verdade quanto o orquestrador. (4) **A versão do template congela na instância**: o primeiro passo fixa `template_version` para a execução inteira; um passo posterior sob versão diferente é aceito e registrado em log, sem reescrever a instância — uma execução que rodou sob `v3` continua dizendo `v3` depois que `v4` existe.                               |
| 2026-09-06 | Rev. 6: três esclarecimentos de mecanismo abertos pela implementação das Fases 2 e 3 (nenhuma decisão F1–F24 tocada). (1) **`suspend` ganhou função de serviço, rota e `DecisionOutcome.SUSPENDED`**: §6.2 dá ao coordenador duas transições (`qualquer → suspended`, `suspended → queued`) que não tinham como acontecer, porque nenhum caminho de código as executava — e suspender agora **para o relógio da fila** (`queued_at` e `assignment_due_at` zerados), já que um item parado não pode acumular idade contra um prazo que ninguém consegue cumprir. (2) **O teto por pessoa e o teto da área são motivos diferentes de exclusão** no `candidates_snapshot` (`member_limit` × `policy_limit`), no lugar do `at_max_open_items` único do `lb-1`: "a área limita todo mundo em cinco" e "ela definiu o próprio limite em dois" são respostas diferentes para "por que ela foi pulada?". (3) **A varredura de indisponibilidade também devolve item cujo executor foi retirado pelo campo nativo de assignees** (§12): o sinal cobre a remoção de uma linha, mas o caminho da própria aplicação apaga assignees como queryset e não dispara sinal nenhum — a varredura horária é a rede que fecha a divergência. |
