...
- [ ] `audit_organizational_routing` sem violações num dump do banco de `stage`. **Continua aberto**: exige um dump real, que a sessão de agente não tem. Verificado nesta sessão (07/09) contra um workspace vazio no banco local (sem dump): `python manage.py audit_organizational_routing --workspace audit-empty-ws` → `No routing violations found.` (exit 0) — prova só que o comando roda e recusa nada, não fecha o critério. Comando exato para a manhã rodar contra o dump: `python manage.py audit_organizational_routing --workspace <slug>` (acrescentar `--write` só depois de ler o relatório).
...
- [x] `pnpm check:types --filter=web` limpo. **Fechado nesta sessão (07/09, SG)**: estava marcado `[ ]` em D0.6 e no README por premissa errada — `pnpm --filter web check:types` isolado falha (5126 erros `Cannot find module '@plane/…'`) porque `check:types` depende de `^build` (os pacotes do monorepo) e o pacote nunca é construído sozinho; `pnpm check:types --filter=web`, que passa pelo turbo e constrói as dependências primeiro, é o comando que a sessão de agente consegue rodar e o que fecha o critério. Executado nesta sessão: exit 0 em 92 s. A correção da premissa nos três lugares que a afirmam (D0.6 §Aceite, README do plano, HANDOFF-PROMPT.md, AGENTS.md §Token Efficiency) é da Sessão S0, que é dona desses arquivos; esta sessão não os toca.

Data do gate: \_\_\_\_ (falta só a auditoria contra o dump de `stage`; os demais critérios estão fechados com evidência acima)
...
### Evidência desta sessão (07/09/2026, SG — gates verificáveis)

Ambiente: `HANDOFF-PROMPT.md` §Ambiente local (PostgreSQL 16, Redis, venv), sem
Docker. Base: ponta do PR #15, `31d35e2b` (M1 do plano da madrugada).

| Comando                                                                                      | Resultado                                                                                                                                                                                           |
| -------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `pytest plane/tests/unit/orca -q -m unit -p no:cacheprovider --create-db`                    | `767 passed, 255 deselected` em `1541.04s` (25m41s). **Este é o baseline da noite** — a Sessão SI compara a suíte na árvore integrada contra este número; nenhuma queda é aceitável sem explicação. |
| `python manage.py makemigrations --check --dry-run`                                          | `No changes detected`, exit 0.                                                                                                                                                                      |
| `python manage.py migrate` → `python manage.py migrate db 0134` → `python manage.py migrate` | Ida e volta limpa: desaplica `0135`–`0138` e reaplica na ordem, exit 0.                                                                                                                             |
| `pnpm check:types --filter=web`                                                              | Exit 0 em 92 s.                                                                                                                                                                                     |
| `pnpm --filter web check:lint`                                                               | Exit 0, `733 warnings, 0 errors` (pré-existentes, nada desta sessão), 861 ms.                                                                                                                       |
| `pnpm --filter @plane/i18n check:sync`                                                       | 19 locales em 100 %, 2 s.                                                                                                                                                                           |
| `python manage.py audit_organizational_routing --workspace audit-empty-ws`                   | `No routing violations found.`, exit 0, contra um workspace vazio criado nesta sessão (sem dump de `stage`).                                                                                        |

**Não verificado nesta sessão** (exige staging ou dump — ver bullet acima e
§6 do plano da madrugada): `audit_organizational_routing` contra dados reais.
