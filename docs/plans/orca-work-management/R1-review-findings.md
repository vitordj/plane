...
**Os seis S2 estão fechados desde 07/09.** O que segue é o registro de por que
eles vieram primeiro, e continua valendo para ler os S3 que restam.

**Três achados tinham prazo, e era o prazo que os ordenava, não a severidade.**
...
- [x] **R1.A1** — Re-POSTar a **mesma** área num item atribuído devolve o item à fila e apaga o executor, respondendo 200. `app/views/organizational_unit.py:583`. **Corrigido em 07/09** portando para `set_responsibility` o early return que a rota pública já tinha em `_place` desde a Fase 1: mesma área, sem modo e sem executor pedidos, devolve o estado atual e não grava decisão. Um chamador que nomeia modo ou executor não está perguntando "esta área responde?" e continua alocando — é o que a rota `organizational-unit-assign/` faz, e ela tem outro nome de propósito.
- [x] **R1.A2** — Rebaixamento a Guest do workspace é capturado como `baseline_role`; sair da área deixa acesso residual ativo. `app/services/orca/org_unit_reconciler.py:436-448`. **Corrigido em 07/09** pela correção (1) que a revisão propôs: quando o cap do papel de workspace é o que baixou o alvo, `last_applied_role` passa a acompanhar o papel efetivo em vez de ficar com o antigo. Dois lugares — o ramo `ACTION_NONE` de `_apply_change`, e a guarda em `reconcile_access` que antes nem chamava `_apply_change` nesse caso. A frase da decisão M3, de que `baseline_role`/`last_applied_role`/drift ficam intocados, volta a ser verdade sem defeito dentro.
...
- [x] **R1.A4** — Guest do workspace lê o e-mail de todos os membros de todas as áreas. `app/serializers/organizational_unit.py:92` e `views:269`. **Corrigido em 07/09**, espelhando o padrão do core: o serializer aceita `show_email` e a listagem o calcula pelo papel real do chamador no workspace, Member para cima. O resto da linha continua visível — que uma área tem gente não é segredo. A rota `effective-access/`, que a revisão trata em A15, continua aberta a Guest.
- [x] **R1.A5** — Guest com API key enumera todas as áreas e todos os projetos que elas cobrem. `api/views/orca/units.py:85`. **Corrigido em 07/09** pela correção que a revisão propôs: `UnitListEndpoint` filtra os vínculos pelo mesmo predicado da rota nativa de projetos — membro do projeto, ou projeto público no workspace — com Admin de workspace vendo tudo, e `distinct()` porque o predicado junta `ProjectMember`. A área continua listada com `projects: []`. **Uma ressalva sobre a evidência da revisão**, que não muda a correção mas muda o tamanho do problema: `Project.network` tem default **2**, "visível a todo o workspace", e os projetos da saída que a revisão transcreveu estavam nesse default — a rota nativa também os mostraria. O vazamento real é de projeto `network=0`, e é ele que o teste de regressão exercita, depois de o primeiro teste ter passado contra o código sem correção e ter denunciado a própria premissa.
...
- [x] R1.A5 e R1.A6 corrigidos antes de `ORCA_PUBLIC_API_ENABLED=1` em produção. Os dois fechados em 07/09, antes de a flag ser ligada em lugar nenhum.
