...
**Aberto de propósito.** Esta sessão não tem staging nem um banco com dados
reais, então não há medição e **não** se cria `OrcaExecutiveSnapshot`. Fecha
só depois de medir 5.1 em staging (>2 s). Sem essa medição, qualquer tabela
de snapshot seria invenção.
...
- [ ] 4 itens `[x]` ou 5.2 `[-]` com medição. (5.1/5.3/5.4 `[x]`; 5.2 espera staging.)
- [x] Cada número da página bate com uma consulta SQL reproduzível anotada em `docs/orca-executive-metrics.md`.
