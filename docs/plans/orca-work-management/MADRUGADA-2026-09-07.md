...
## 10. Epílogo — o que realmente aconteceu (escrito em 07/09, 15h UTC)

Este arquivo era um plano. Ficou no repositório porque o RFC §4.2 rev. 7 cita
as decisões M1–M12 dele, e uma decisão registrada precisa de um endereço. O que
segue é o resultado, para que ninguém leia o plano como se fosse a história.

**Cinco das sete sessões entregaram. Duas falharam, por motivos diferentes, e
os dois motivos valem mais que os itens perdidos.**

| Sessão | Resultado |
| --- | --- |
| S0 saneamento | entregue · PR #16, mesclado |
| SR revisão adversarial | entregue · PR #17, mesclado · 20 achados, nenhum S1 |
| SG evidência de gate | entregue · dentro do PR #18 |
| SA backend 2.1 + 2.2 | entregue **inteiro**, inclusive `transfer` e `decisions`, que a linha de corte M11 permitia sacrificar |
| SB frontend 2.3-mínimo | entregue · B1 a B7, inclusive o B7 cortável |
| SC alertas 2.4 | **falhou** · rejeitada pelo limite de uso de 5 horas ao ser disparada às 06:45 UTC, com zero tokens consumidos |
| SI integração | **falhou** · nasceu sem repositório anexado |

**A falha da SC é sobre orçamento, não sobre engenharia.** Quatro sessões Opus
em paralelo por três horas esgotaram a janela de cinco horas antes de a quinta
ser disparada. O plano tratou tempo como o recurso escasso e agendou por
relógio; o recurso escasso era cota. Um plano futuro com sessões paralelas
precisa contar cota, escalonar os disparos, ou aceitar que a última da fila é a
que morre. A SC foi redisparada às 10:24, depois do reset, e entregou.

**A falha da SI é um defeito de despacho que vale registrar por extenso.** Uma
sessão criada por `create_trigger` **não herda as fontes** da sessão que a
criou: herda o ambiente e nada mais. Ela subiu sem repositório, gastou 321 mil
tokens sem poder tocar em nada e terminou com status de sucesso, porque do
ponto de vista do agendador ela rodou. Quem for despachar trabalho agendado tem
de anexar a fonte explicitamente, e desconfiar de "sucesso" que não produziu
branch. A integração foi refeita à mão pela sessão que planejou a noite.

**O que a noite provou sobre o próprio processo, e que já está no
`HANDOFF-PROMPT.md`:** a sessão de agente roda `pnpm install`, `check:types`
pelo turbo, `check:lint`, `check:format` e o `check:sync` do i18n. A premissa
contrária, que estava em três documentos, custou um critério de gate marcado
como impossível durante dias.

**Duas decisões desta lista foram contrariadas pelos fatos, e é assim que
deveria ser:**

- **M11**, a ordem de corte, não precisou ser usada. Nada foi cortado.
- **M8**, adiar o vitest, continua de pé e virou item próprio no arquivo da
  Fase 2, com a justificativa.

**O que a noite não mediu, e um plano futuro deveria:** ninguém abriu a
interface. Não havia ambiente com dados, então a aba Trabalho foi verificada
por tipos, lint, formato e i18n, e por nenhum olho humano. Isso é um item de
staging, e continua aberto.
