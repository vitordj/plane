...
"""
Work whose executor is no longer there (items 3.4 and 3.6).
...
from .conftest import issue_reassign_url
...
@pytest.fixture
def executor(covered_unit, project, add_member, grant_manual_access, plain_user):
...
class TestTheSweep:
    ...
    def test_the_coordinators_are_told(
        ...
        add_coordinator(covered_unit, second_user)
        send_them_away(executor)
...
class TestTheCommand:
    def test_dry_run_writes_nothing(self, availability_on, assigned_issue, send_them_away, executor):
        send_them_away(executor)
        stdout = StringIO()

        call_command("sweep_unavailable_executors", stdout=stdout)

        assert IssueOrganizationalUnit.objects.get(issue=assigned_issue).routing_state == RoutingState.ASSIGNED
        assert "Found 1" in stdout.getvalue()

    def test_write_is_refused_while_the_flag_is_off(self, assigned_issue, send_them_away, executor, settings):
        settings.ORCA_AVAILABILITY_ENABLED = True
        send_them_away(executor)
        settings.ORCA_AVAILABILITY_ENABLED = False

        with pytest.raises(CommandError, match="Availability is disabled"):
            call_command("sweep_unavailable_executors", write=True)
...
class TestNothingMovesWithoutADecision:
    ...
    def test_accepting_a_suggestion_is_an_ordinary_assignment(
        ...
        reassign like any other choice, and only the reason on the decision
        ...
        response = admin_client.post(
            issue_reassign_url(workspace_with_members.slug, project.id, assigned_issue.id),
            {"executor_id": str(second_user.id), "reason": "accepted_suggestion"},
            format="json",
        )
        ...
        assert latest.trigger == "ui_coordinator"
