...
class MyAvailabilityEndpoint(AvailabilityFeatureMixin, BaseAPIView):
    ...
    def delete(self, request, slug, pk):
        ...
        if row is None:
            return orca_not_found("ORG_AVAILABILITY_WINDOW_NOT_FOUND")
...
class MemberAvailabilityEndpoint(AvailabilityFeatureMixin, BaseAPIView):
    ...
    def get(self, request, slug, workspace_member_id):
        ...
        if not self._may_read(request.user, member):
            return orca_error("ORG_NOT_AVAILABILITY_MANAGER", status.HTTP_403_FORBIDDEN)
    ...
    def _may_read(self, actor, workspace_member) -> bool:
        if workspace_member.member_id == actor.id:
            return True
        return may_manage_member_availability(actor, workspace_member)
...
def _create_absence(request, workspace_member):
    ...
    """
    raw_from = request.data.get("unavailable_from")
    if not raw_from:
        return orca_error("ORG_AVAILABILITY_FROM_REQUIRED")
    unavailable_from = parse_datetime(str(raw_from))
    if unavailable_from is None:
        return orca_error("ORG_AVAILABILITY_INVALID_INTERVAL")
...
class MembershipAllocationEndpoint(AvailabilityFeatureMixin, BaseAPIView):
    ...
    def put(self, request, slug, unit_id, pk):
        ...
        # MembershipAllocationSettings has no workspace column on this fork
        # (0141); the original passed workspace_id here. Do not invent one.
        row, _ = MembershipAllocationSettings.objects.get_or_create(membership=membership)
