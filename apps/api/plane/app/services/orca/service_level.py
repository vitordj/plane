...
def record_from_resolution(
    issue, link, resolution, *, actor=None, reason="", assignment_due_at=None, completion_due_at=None
):
    """
    @description The allocation path's shorthand: take the assignment date the
    link now carries (or the one the caller sent) and the policy that produced
    it. Called from the one place that writes routing state, so a work item
    cannot end up with a due date the service level never heard about.
    @param issue: The work item.
    @param link: Its ``IssueOrganizationalUnit``, after the state change.
    @param resolution: The ``PolicyResolution`` in force, or ``None`` when the
        caller named the person (``explicit``).
    @param assignment_due_at: Explicit deadline from the request, used when
        the link has none (assigned items do not write the SLA onto the link).
    @param completion_due_at: When the work itself should be done.
    @returns: The ``IssueServiceLevel``, or ``None``.
    """
    due = link.assignment_due_at if link.assignment_due_at is not None else assignment_due_at
    if resolution is None:
        source = ServiceLevelSource.MANUAL
        version = ""
    else:
        source = SOURCE_FOR_POLICY.get(resolution.policy_source, ServiceLevelSource.MANUAL)
        version = resolution.policy_version or ""
