...
def unusable_executor_links(now=None):
    ...
    """
    from plane.app.services.orca.availability import unavailable_workspace_member_ids
    ...
    links = list(
        ...
        ).select_related("issue", "organizational_unit", "primary_executor", "project", "workspace")
    )
    if not links:
        return []
    ...
    away_rows = unavailable_workspace_member_ids(member_row_of.values(), at=now)
...
def return_unusable(link):
    ...
    from plane.app.services.orca.alerts import ALERT_EXECUTOR_UNAVAILABLE, notify
    ...
    # Same audience as an overdue item: a coordinator needs to know something
    # is waiting for a decision. Re-read with the relations ``notify`` uses.
    fresh = IssueOrganizationalUnit.objects.select_related(
        "issue",
        "project",
        "organizational_unit",
        "workspace",
    ).get(pk=result.link.pk)
    return notify(fresh, ALERT_EXECUTOR_UNAVAILABLE)
