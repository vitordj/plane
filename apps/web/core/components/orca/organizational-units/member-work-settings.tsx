    ...
    <div className="bg-custom-background-90/40 space-y-4 rounded px-4 py-3">
      <div className="flex items-start justify-between gap-4">
        <div className="min-w-0">
          <p className="text-custom-text-200 text-xs font-medium">{t(`${OU}.allocation.accepts_new_work`)}</p>
          <p className="text-custom-text-400 text-[11px]">{t(`${OU}.allocation.accepts_new_work_help`)}</p>
        </div>
      ...
      <div className="flex items-start justify-between gap-4">
        <div className="min-w-0">
          <p className="text-custom-text-200 text-xs font-medium">{t(`${OU}.allocation.max_open_items`)}</p>
          <p className="text-custom-text-400 text-[11px]">{t(`${OU}.allocation.max_open_items_help`)}</p>
        </div>
        <Input
          type="number"
          min={1}
      ...
      <AvailabilityForm workspaceSlug={workspaceSlug} workspaceMemberId={workspaceMemberId} canEdit={canEdit} />
    </div>
  );
});
